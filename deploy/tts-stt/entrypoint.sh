#!/bin/sh
# Launch both halves of the tts-stt speech service in one container. Kokoro (TTS) is the
...
# missing config would make llama-swap exit and take the whole container, and TTS, down).
#   • Kokoro TTS server on :8801 (the wall forwards /tts* here; the api reaches TTS here)
...
# to system python3 (Kokoro-on-espeak) when the venv was skipped or failed to build, so read-aloud
# always runs. The `import kokoro_onnx, misaki.en` probe rejects a half-built venv (misaki is the
# only thing the venv adds over system python).
TTS_PY=python3
