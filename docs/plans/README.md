...
| `EXTERNAL_VIDEO_INGESTION_PLAN.md` | **In progress** — analysed YouTube videos → an isolated, embedded, searchable corpus (never the knowledge graph). Waves A–B shipped (migrations 0133–0134, timeline windower, `embed_external_source`, `analyze_stream` write-through, the `search_external` + `check_channel` jerv tools); Wave C (a recurring Jerv Task for scheduling) open. |
