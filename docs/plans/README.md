...
| `LONG_MEDIA_ANALYSIS_PLAN.md` | **Scheduled** — move long `analyze_stream`/`analyze_video` runs off the interactive turn: W1 cancel-safe async ffmpeg subprocesses (fixes Stop-orphans-ffmpeg), W2 job-backed long analysis, W3 auto-resume the finished card into the chat. All open. |
