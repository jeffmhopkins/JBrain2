...
| `STREAM_ANALYSIS_PLAN.md` | **Scheduled** — `analyze_stream`: jerv pulls frame(s) + optional whisper audio from a video **URL** (live or VOD) via yt-dlp + ffmpeg, reusing the `analyze_video` map→fuse→reduce core. W1 (stream sampler + yt-dlp dep) · W2 (shared reduce core + tool) · W3 (docs + egress red-team); all open. |
