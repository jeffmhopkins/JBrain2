...
| `EXTERNAL_VIDEO_INGESTION_PLAN.md` | **In progress** — analysed YouTube videos → an isolated, embedded, searchable corpus (never the knowledge graph). Wave A shipped (migrations 0133–0134, timeline windower, `embed_external_source`, `analyze_stream` write-through, the `search_external` jerv tool); Wave B (`check_channel` tool) + Wave C (a recurring Jerv Task for scheduling) open. |
