...
| `DEEP_RESEARCH_VIDEO_SOURCES_PLAN.md` | **Scheduled** — a `sources` knob on the shipped `deep_research` tool so a run can draw from the owner's external video library (`external_sources`/`external_source_chunks`) instead of, or ahead of, the open web: `web` (default, unchanged), `library` (exclusive), `library_first` (library-primary, web fills the gap round). Structural — `sources` only picks the gather/refill child persona (`research` vs. a new `research_library`); the pipeline, budget, and report are otherwise unchanged. Not the deferred KB-scoped research (notes/wiki stay out of scope). DV1 (routing + flag) / DV2 (steering + provenance + red-team) / DV3 (conditional GUI mock gate) all open. |
