...
> **Status:** In progress · **Last verified:** 2026-07-30 · **Waves:** W1✅ W2✅ W3◻️
...
| **W3 — single-source primitives + table + routing ✅ (code landed on-branch; ◻️ = merge)** | Windowed single-video read: `read_external_video(from_ms=…)` truncates on a window boundary and hands back the exact `from_ms` to resume at, so an exhaustive reader sweeps the whole 80k-char transcript across calls (`.tool` v1→v2). Enumeration-mode clause in `research_library.prompt` (v1→v2): a "list ALL" brief overrides the "stop early" guidance and reads the full source. Routing (`jerv.prompt` v27→v28 + `deep_research.tool` v3→v4 + `deep_produce.tool` v1→v2): a task about a specific analysed video uses `sources=library_first`, **never `web`** (the observed `deep_produce(table, sources=web)` error), and a structured-artifact ask uses `deep_produce(output_kind=table, sources=library_first)`. The staged pipeline (W2) supplies the extract→answer/fact-check plan under it. | a windowed read reaches the final window across successive `from_ms` calls (and past-the-end says so); an enumeration brief is told to sweep the full source; routing steers a specific-video task to a library source; all `.prompt`/`.tool` digests re-pinned. |
