...
Living docs live by **sub-kind directory**: `reference/` (how the system is —
architecture, standards, models), `runbooks/` (how to operate the box), plus
`README` and `ROADMAP` at the top level (the map + the phase status) and this
doc. They do not move between kinds. Plan docs are the `*_PLAN.md` family and any
doc that names waves (e.g. a `*_CONTRACT.md` that sequences work); active plans
live in `plans/`.
...
currency is governed by `reference/DESIGN.md`; `research/` holds design dossiers
feeding a plan (see Homes). Both are kept current, neither climbs the plan ladder.
...
2. **Commit to the roadmap** → flip to `Scheduled`, `git mv` from `proposed/`
   into `plans/`, add waves + a `ROADMAP.md` entry + a `plans/README.md` row.
3. **A wave merges** → flip to `In progress`, tick that wave's marker (header +
   body), bump `Last verified`. Same PR as the wave.
4. **The last wave merges** → flip to `Shipped`, `git mv` from `plans/` to
   `archive/`, carry any residual/open/deferred items into `ROADMAP.md` so nothing
   is lost, update `archive/README.md` + `plans/README.md`. Same PR. **A feature
   is not done until its plan is archived.**
...
## Homes — where each kind and state lives

Docs are sorted first by **kind** (into sub-directories), then by **state**.

| Directory | Holds | Never holds |
|---|---|---|
| `docs/` (top level) | `README`, `ROADMAP`, `DOC_LIFECYCLE` — the map + entry points. | Anything else. |
| `docs/reference/` | Living design references + a Superseded-but-still-cited reference doc (with a banner). | Plans, runbooks, terminal states. |
| `docs/runbooks/` | Living operational runbooks. | Plans, design references. |
