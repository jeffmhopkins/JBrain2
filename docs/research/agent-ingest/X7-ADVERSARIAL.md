...
| 7 | **Cross-subject attribution is force-staged** | `arbiter.py:131-132`; `intent.py:57`; validator emits `cross_subject_link` at review severity (`intent.py:~185`) | Attributing Mom's medication to the owner. Treated as a *leak*, not an error | **Depends entirely on tool design.** gpt-oss-120b is genuinely good at *detecting* cross-subject (V2 §15: "it tagged the correct non-owner entity + `cross_subject` every time"). It is the *disposition* it misses. Detection without a deterministic router is worth little. |
...
| 11 | **Duplicate-fact collapse** | `arbiter.dedup_intent_facts` (~`arbiter.py:487`), keyed excluding statement AND object | The "account address explosion": one value rendered nine ways filed nine `attribute_collision` cards | **Reintroduced.** A tool-calling writer emits one `write_fact` per thought; nothing dedups across calls unless the tool does. |
...
- **Silent corruption with no arbiter.** The arbiter is not one gate, it is ~10:
  ...
  routing, the domain floor, shape checks (`pipeline.py:~1875-1929`), the
