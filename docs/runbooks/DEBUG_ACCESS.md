...
> **Status:** Living · **Last verified:** 2026-09-06
...
| `POST /sdr/listen-probe` | **Does the numpy demodulator really work on this radio?** The twin of `spectrum-probe` one layer over: that one retires the claims the spectrum engine is written against, this retires the ones `deploy/sdr/demod.py` is. Both exist for the same reason — listening is an owner route and the audio is an MP3 stream, so before this the only way to know whether the demodulator worked on real hardware was for the owner to press play and listen, and an owner with no terminal cannot be the test harness for their own box (CLAUDE.md #10). It starts a genuine listening session, holds it for a few seconds and gives the radio back, returning a **verdict, not a dump**: `ok`, a one-line `summary`, and `findings` naming any claim that did not hold. Five of them, none provable against the synthetic signals `test_sdr_demod.py` uses. **Which engine ran** — the listen pipeline drops to `rtl_fm` at RUNTIME when a radio will not open for our own samples, which is right and whose silence is not: on `rtl_fm` there is no tuning view at all and nothing else says why. **Whether the station is where the offset tuning says** — the radio sits `LISTEN_OFFSET_HZ` above it and the mixer takes that back out, so `view.strongest_offset_hz` is the number that catches the two being out of step, which on a narrowband channel is silence from code that reads correctly in both places. It is measured as the **midpoint of the 6 dB shoulders**, the same rule `frontend/src/sdrTuning.ts` draws with and deliberately not the argmax: an FM carrier's top is flat, so the loudest bin is wherever the noise peaked across a plateau a hundred bins wide, which on a 32 kHz row is an answer a third of the view out. **Whether the audio is a signal rather than silence or a rail** — a peak at 0.0 is a chain not connected and a peak at 1.0 is one clipping, and both look like "it ran" from outside; the peak is sampled ALONGSIDE the frames rather than once at the end, since a channel loud for four seconds and quiet for the fifth would otherwise read as silent. **How many USB buffers the driver threw away** — `overflows` is the one measurement no fake radio can produce, and the one worth re-reading now that the same stream has to run continuously for audio rather than in frames for a picture: on a waterfall a dropped buffer is one row slightly wrong, on audio it is an audible click. **TAKES A RADIO** for those seconds through the same lease as everything else — a `409` names the holder, and the session is released in a `finally` because the owner has no terminal to free a radio a diagnostic walked away from. **MEASURED 2026-09-05, the first captures through the numpy demodulator.** The engine came back `iq` on every run with **zero USB overflows** across ~30 s of continuous 2.4 MS/s capture at `QUEUE_BUFFERS = 4` — the risk no fake radio could exercise — at 10.0-10.1 fps, with the offset tuning landing the station within **48 Hz** on a 93.75 Hz bin. It also caught two defects, both invisible without hardware. **The listening path never set the tuner gain**: `radio.Radio.open` does not touch it and `_start_iq_listen` did not either, so a session ran at whatever librtlsdr left the dongle at (0.0 dB, the bottom of 0-49.6) while `rtl_fm` hands the tuner to its own AGC — 162.550 read -64.5 dBFS; with AGC restored it reads **-47.3**, a real 17 dB of level the chain had been throwing away. The "+17 dB over the floor, dead on frequency" this row claimed alongside it was not a signal — see the row below. **And the probe's own clipping test was wrong**: `peak >= 0.999` on FM is not a clipping test at all, because a discriminator turns every burst of noise that momentarily overpowers the carrier into a full-scale impulse, so one sample in 1600 pins the peak while the rest is a good voice. `clipped_fraction_max` replaced it, and its threshold is measured rather than guessed: 162.550 clips 1.0-1.25% with RMS 0.40, while 96.5 / 99.3 / 104.1 on wbfm at **31-40 dB SNR clip 0.0%** with peaks at 0.37-0.42 — so the clicks are the noise and the deviation scaling is right. (162.550's "14 dB SNR" was our own filter shape and the channel was empty; the threshold stands, and 1% at the rail is now known to be what pure noise through a discriminator does.) It did not move with the tuner gain either (1.00% on AGC, 1.19% at 40 dB), because a discriminator is blind to amplitude — which is also why `audio_peak` can never answer a question about signal strength and `view.snr_db` is the number that can. **MEASURED 2026-09-06, and it is why this probe now has a fifth claim.** The owner reported static on 162.550 while the tuning strip drew a strong, perfectly centred station reading -47.4 dBFS and "On centre", and this probe agreed: `ok: true`, 15.7 dB SNR, 10.1 fps, no overflows. It looked at the time as though there was **nothing on the air** — an `rtl_power` sweep of 162.3-162.7 found a flat band — and pure noise pushed through the same chain in a test reproduced the probe's numbers to within two decibels — 13.7 dB "SNR", RMS 0.37, 1.3% at the rail. The cause was in `demod.py`: the front end was the channel filter as well as the anti-alias one, with its cutoff asked for at the passband edge, and a windowed sinc's cutoff is its SIX DECIBEL point — so the response sagged from 0 dB at DC to -4.7 dB at the edge of the channel, and an empty channel arrived at the strip as a hump centred exactly on the tuned frequency. Every instrument agreed with every other, and all of them were reporting the shape of our own filter. The front end is now anti-alias only and flat across `view_half_hz`, a separate channel filter does the selectivity, and **the picture is taken between them** — which is also what finally makes the shaded passband mean something, since you can only see a signal fall outside a filter if the picture is wider than the filter. **And flattening it broke wide FM the opposite way, which the same probe caught within the minute.** With the picture taken at the 240 kHz IF, the anti-alias filter only had to reach its stopband by 144 kHz — so everything from 120 to 144 kHz folded back into the row, and a carrier 200 kHz out, which on the FM broadcast raster is exactly where the NEXT STATION sits, arrived at -40 kHz at full strength. 104.1 reported its strongest bin 100 kHz off centre and 96.5 read 2.8 dB over a floor its own neighbours were holding up: a false negative on a strong station, from the fix for a false positive on an empty one. The picture is now drawn at `demod.view_rate_hz`, which is twice the IF for wide FM and the IF itself for every other mode, and is taken ABOVE the decimation that was folding. Demodulating wide FM at 480 kHz would have done it too and was measured at 21.7% of a core against 6.1%, because an audio filter's length goes with the rate it runs at; taking only the 4 096 samples the transform consumes costs 8.1%. **And with the picture finally honest, it found the fault underneath both of them: the offset tuning's sign was inverted, and had been for the life of the I/Q listen path.** `_Mixer` shifts the spectrum DOWN by `offset_hz`, so what reaches DC is what sat ABOVE the tuned centre — and `listen.py` tuned the radio ABOVE the station, which put it at `-offset` and the mixer then moved it to `-2 * offset`. 480 kHz from DC, past every filter in the chain. MEASURED: a carrier at +240 kHz reaches DC at +30.1 dB and one at -240 kHz reaches it at -35.1, sixty-five decibels down, so the station was not attenuated but GONE and the discriminator spent its life on the empty spectrum 480 kHz above it. Proved on the box before a line was changed, by asking for the frequency the bug made reachable: **99.3 read 3.7 dB over the noise, 99.3 minus 480 kHz read 21.8 dB with four times the audio**; 104.1 read 3.6 and 104.1 minus 480 kHz read 14.0. This is the fault the owner reported as static, and it survived a suite of eleven on-air probes and a hundred and seventy unit tests because **every one of them measured LEVEL** — and an FM discriminator is blind to amplitude, so fed nothing at all it emits noise at FULL SCALE: peak 1.0, RMS 0.4, a busy meter, a moving tape, a plausible waterfall. `test_the_audio_carries_the_station_and_not_noise` is the check that was missing, and it asks the only question that separates a working chain from a dead one: not how loud the audio is, but WHAT IS IN IT. **VERIFIED ON AIR after the fix**: 162.550 reads 39.3 dB SNR at -12.6 dBFS, 422 Hz off centre, `ok: true`; 96.5 reads 24.2 dB dead on centre, 99.3 reads 22.1 and 104.1 16.6; 102.5, which has no station, reads 10.6 dB with its strongest bin 175 kHz off centre and is called below the FM threshold; 146.94, an idle repeater, reads 3.2 dB and "nothing is transmitting here", which is what an idle repeater is. **And the sweep's disagreement, which was NOT a second engine fault**: an `rtl_power` sweep of 162.3-162.7 reported a flat band while the I/Q engine measured 39.3 dB there minutes apart. The raw CSV settles it — 162.550 is the strongest bin in the row, 9.8 dB over its median — so rtl_power received the station perfectly and the sweep was running on **AGC**, because `-g` was passed only when a caller named a gain. Three runs each way: on AGC the floor wandered and the reduction reported a station at **162.35**, which is not a NOAA channel and not anything, while 162.550 appeared on some runs and not others; at a fixed 30 dB the floor holds to 0.2 dB, 162.550 is found every time at ~29 dB over it, and 162.35 is gone. Both measuring paths now default to `listen.MEASURING_GAIN_DB` — the live spectrum's I/Q engine never set a gain at all, so its dB scale was a property of whatever the previous session left behind. Listening keeps AGC, where loudness is the point and a moving gain costs nothing. `snr_db` is a true reading from here on, and `CHANNEL_SIGNAL_DB` / `FM_THRESHOLD_DB` are the findings that say so: **"nothing is transmitting here"** below 6 dB over the floor, and **"below the FM threshold"** below 12 — the one question audio level can never answer, on a chain where noise is a full-scale signal. **`--transcribe` is the check that is not a measure of level, and it is here because everything else in this row is.** An FM discriminator differentiates phase and is blind to amplitude, so a chain demodulating the wrong piece of spectrum reads healthy on every other number this probe returns — which is exactly how the inverted offset above survived. Words are the only evidence that separates a receiver tuned to a station from one tuned to nothing, so the sidecar taps the DEMODULATED PCM (only when asked, into a bounded buffer) and the api hands it to whisper. **Read `ok` and `view.snr_db` FIRST and the transcript second**: whisper hallucinates fluently on noise, so a transcript is evidence only when it matches what the station is known to be transmitting. Note it tests a different path from `POST /sdr/capture` below, which spawns `rtl_fm`: a capture proves the BAND is alive, this proves `demod.py` is. **`--band` is the sixth claim, and it is one no earlier build could make**: what ELSE was on the air, measured from the very same capture that produced the audio. A spectrum session and a listening session are one dongle apiece, so before W3 (`../plans/SDR_RECEIVER_CONVERGENCE_PLAN.md`) answering "what is on 2 m" meant giving up "listen to 146.94". One capture now feeds both — the channel demodulated and the whole 2.4 MHz around it transformed — and `band.peaks` is that row's signals, strongest first, in MHz with how far each stands over the noise around it. Read `band.rows`: zero means no wideband row arrived, which is a probe that asked too late or an engine (`rtl_fm`) that draws nothing, NOT a quiet band. Off by default because the transform is real work — 11.4% of one core, re-measured 2026-09-06 — that the sidecar skips entirely while nobody has asked for it. Note the band row is centred where the RADIO is, `LISTEN_OFFSET_HZ` below the station, so its range is not symmetric about what you asked to hear; the row says its own edges. **And `band.peaks` is EMPTY unless the tuner gain is pinned, which is a finding this probe made on its first hour of use.** Listening keeps AGC on purpose — loudness is the point there — and MEASURED 2026-09-06 on 162.550 the tuner's own loop ran the carrier to **-7.0 dBFS**, at which the R820T2's front end answered with a symmetric spur comb at **±55.5, 111, 166 and 222 kHz**: seven phantom stations, none of them on NOAA's 25 kHz raster, every one of them a perfectly correct `peaks.find` reading of a receiver overloading. The same radio with `--gain 30` found **one**, 27.1 dB over the floor at +0.6 kHz — agreeing with `spectrum-probe` over the same span (28.6 dB over) to within 1.5 dB. So the picture is still drawn under AGC, its levels are relative and `band.gain_db: null` says so, and `band.note` names why there are no peaks; **ask with `--gain 30` to measure the band.** Read `band.peaks` with B4 in mind (`../plans/SDR_RECEIVER_CONVERGENCE_PLAN.md`): the peak folder is still gap-based, so two stations one raster apart can arrive as one — a listening session carries no channel raster to fold against, since only a swept range has ever had one. CLI: `scripts/debug-connect.sh listen-probe --mhz 146.94`, `--mode wbfm --mhz 96.5` for the broadcast dial, `--gain 40` to hold the tuner still and compare two runs, or `--band` to see the dial the station is on. |
