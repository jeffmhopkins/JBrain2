...
| Route | Purpose |
|-------|---------|
| `GET /whoami` | Token label, kind, and the fixed scope set (`llm.complete`, `sql.read`, `logs.read`, `llm.routing`, `llm.gateway`, `host.read`, `host.metrics`, `web.fetch`, `jmolt.sim`, `embed`, `jmolt.status`). `llm.gateway` is the gateway surface — load/unload, the served `-c`/`-np`, launch flags via the allowlist, props/slots/metrics, prime. |
...
| `GET /jmolt/status` ⚠️ | *Not yet exercised against the live box — CI-covered only.* Why jmolt did or did not run: registered, account state, night enabled + hour, kill, autonomy, engine, verify-fail streak, last night, last drip sweep, integrity heartbeat — plus **`would_run_now`** and **`blocked_by`**, so "it didn't run" resolves to one line instead of a hunt through five settings screens. Written after exactly that question could not be answered from this surface: on **2026-08-30 the nightly run did not happen** (no `agent_sessions` row, no outbox rows) and `GET /logs/api` is uvicorn's access log, so it carries no `jmolt_night` events. Everything deciding whether a night starts lives in `app.settings`, which this console must NEVER read — that table holds the Moltbook bearer key and the Gmail secret in plaintext — so the answer comes back as switch STATES: booleans, an hour, timestamps, and whether an advisory note EXISTS. Never the key, never the note's text. |
| `POST /embed` ⚠️ | *Not yet exercised against the live box — CI-covered only.* Embed text through the box's own embedder — the same client the knowledge graph and the jmolt claim gate use. Exists for a measured reason: the gate's threshold cannot be chosen by reasoning. Measured 2026-08-30 on jmolt's real duplicate posts, prose lexical similarity reproduces the cold study's 0.07–0.20 band and CANNOT separate a restatement from an unrelated post (the closest unrelated pair scores 0.184 against the most distant restatement's 0.100); triple-space lexical doubles the separation in means and still overlaps by 0.008. Setting the threshold needs real vectors over real pairs, and this is the only way to reach the embedder with no shell. Reads nothing, writes nothing — a pure function of the text handed to it. |
