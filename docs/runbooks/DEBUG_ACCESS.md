...
| `POST /sdr/sweep` → `GET /jobs/{id}` | Sweep a band with `rtl_power` and report what was busy in it — a **background job**, because a five-minute sweep will not survive the tunnel's ~100s edge limit. Returns a waterfall PNG (base64), the occupancy table, and the sweep's own noise floor. **This TAKES THE RADIO** as a real lease with the omnibox icon and Release, so it is refused with a 409 while APRS is logging. A measuring instrument rather than an agent tool, and deliberately so: a detector's thresholds have to be calibrated against THIS box's noise floor and spur pattern — per-box facts nothing documents, the same reason `/grounding` exists for a vision model's coordinate base — and guessing them ships a detector that reports the tuner's own artifacts as stations. **`occupancy` is a fraction of the window, not a peak in dB**: a one-off burst and a channel busy half the hour have the same peak. **`steady`** carries bins that never went quiet — a spur and a carrier held all window are the same measurement, so they are reported rather than dropped. `steady` compares each bin to its NEIGHBOURS' floor, not the sweep's, because "high" is a local claim — band-edge rolloff, spurs and each retune block's own noise all vary across a sweep. **Its threshold is not calibrated**: three 2m sweeps of this box on 2026-09-03 found a quiet band (floor spread 6.7-7.2 dB at any gain, excess over the local floor p99 0.8-2.0 dB) with no steady carrier to set it against, so `steady` reporting nothing here is the honest answer rather than a miss. Setting the number needs a sweep taken while something transmits. **`uncovered`** lists the spans the sweep did NOT measure — a 144-148 run once came back missing 145.872-146.206, a 342 kHz hole across live repeater channels, with nothing saying so; that was the reducer's own block-trimming and the same sweep now returns 1026 contiguous bins with `uncovered: []`. Silence is only evidence where the receiver was listening. **`include_csv=true`** returns rtl_power's own numbers: off by default because it is megabytes, but calibrating the detector against its own summary is circular — and calibrating against the returned PNG is worse, since the waterfall palette is stretched between the 20th and 99.5th percentile of the data, so brightness ratios off it mean nothing in dB. One wrong "1.76x" in this file came from exactly that. `channel_khz` groups adjacent bins into one signal (15 for 2m, 25 for 70cm), by adjacency rather than by snapping to a grid, because real band plans are not anchored at 0 Hz. `409` means another caller holds the single tuner; `503` means this box has no radio (`sdr_url` unset). Poll `GET /jobs/{id}` until `status` is `done`; `png_base64` is a PNG, and `rows`/`csv_chars` are what separate a genuinely quiet band from a sweep that never ran. CLI: `scripts/debug-connect.sh sweep 144 148 --seconds 300 --channel-khz 15`, which submits and polls for you (`--no-wait` returns the job id instead). The CLI writes the PNG — and, with `--csv`, the raw numbers — to files under `--out DIR` and prints their paths in place of the blobs, so the reading is readable. |
...
| `POST /sdr/capture` | **Tune the radio, record a few seconds, and run it through whisper.** The end-to-end proof for the SDR plan's S0b-ii gate, driven from the console so it needs no terminal. Query params only — `frequency_mhz`, `seconds`, `mode` (`fm`/`nfm`/`wbfm`/`am`/`usb`/`lsb`), `gain`, `transcribe` — **never a URL or a host**, and the frequency is bounded to the tuner's real range both here and in the sidecar, so the `stream.py` SSRF guard is neither used nor widened. Read **`peak`/`heard_something` before the transcript**: a dead antenna, a mistuned frequency and a working capture of silence all return audio of the same length, and only the level tells them apart — whisper will confabulate words over noise, so a transcript from a silent capture is an artefact, not a reception. `409` means another caller holds the single tuner; `503` means this box has no radio (`sdr_url` unset). Console: pick **tune — capture + transcribe FM** and enter a frequency. CLI: `scripts/debug-connect.sh tune 99.3 --mode wbfm`. |
