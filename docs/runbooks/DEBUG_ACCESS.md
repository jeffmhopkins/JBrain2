...
| `GET /llm/upstream-logs` | Tail **llama.cpp's** own output, which `gateway-logs` cannot show: a load's per-buffer memory breakdown (`model buffer size` / `KV buffer size` / `compute buffer size`), the Vulkan device report, and the engine's account of a failed load. Reads the history burst llama-swap replays on `/logs/stream/{stream}`; `stream` defaults to `upstream` (all models interleaved) or takes a served model id to isolate one. An empty body means the engine has printed nothing since llama-swap started — usually no load since boot, not a fault. |
