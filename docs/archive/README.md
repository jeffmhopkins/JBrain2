...
| Item | What it is |
|---|---|
| `PROMPT_CACHE_HARDENING_PLAN.md` | The 2026-09 hardening of the jerv prompt cache, from an eight-way parallel review of it. P0 made the cache observable — it was correct and silent, which is how it had shipped inert twice — and P1-P5 fixed the defects that each turned a ~100 ms restore back into a ~125 s prefill: a divergent tool array on every scheduled task, a memo that survived the slot it described, a poisoned file nothing repaired, a keeper that erased its own edge triggers, and fakes that agreed with the docstring instead of testing against llama.cpp. Living successor: `../reference/PROMPT_CACHE.md`. |
...
| `DITOO_PLAN.md` | **Superseded** by `../proposed/ROOM_ENDPOINT_PLAN.md`. Feasibility for driving a Divoom Ditoo (16x16 Bluetooth pixel speaker) from the box; the device was dropped for two Wi-Fi AMOLED endpoints that need no Bluetooth. Kept for four findings that outlive it: the USB question closed with evidence (§1.1), Bluetooth sockets working only in the initial network namespace so a BT sidecar forfeits the `internal: true` egress lock (§4.2), what `update-inner.sh` can and cannot do to the host from the PWA path (§4.3), and the correction that `bluez-alsa` needs no host sound server (§5). §0.2 holds a 2026-09 survey of small pixel/voice hardware. |
...
| `SHOW_THE_WORKING_PLAN.md` | The surfaces that make the exact-math tools checkable, plus the question reaching a plain chat. Every Worked row says what came BACK, not only what was asked (`result_brief` + `stepLedger.ts`; 108 of 127 tools author their answer, the rest render from structured fields, none render blank); a run shows its code in its step (`code_run`, one component for `run_python` and `calculate`, highlighting owned by the component and containment chips checked against the compose file); a computed number cites its working through a `[=n]` marker and a floating popover that renders that same component; and `ask_owner` records its set on the turn when there is no note behind it, granted to curator and teacher and never to a non-owner persona. Records why each gate was made to bite — three tools were caught rendering blank behind a label saying they were fine, and the written-answer escape the owner asked for became a rule rather than behaviour that happened to be there. |
...
| `SDR_RECEIVER_CONVERGENCE_PLAN.md` | **Shipped 2026-09-06.** The receiver stops being four mutually-exclusive session kinds and becomes what every other SDR application is: ONE capture, fanned out to many sinks. Both subprocess engines (`rtl_power` for the picture, and again for the survey) deleted; retune happens in place on a live stream; filter design became a SPECIFICATION (Kaiser with an asked-for stopband, `-55 → -79 dB` of measured leakage) instead of whatever a Hamming window happened to give; AM and SSB gained an AGC and SSB's passband stopped being drawn symmetric. Kept for its measurements, which are the point of it: **every wave was verified on air**, several findings were closed by MEASURING them and declining the proposed remedy (C16's de-emphasis move costs 2.8 dB of shape at a 16 kHz audio rate; C25's ppm cannot be measured with any reference this receiver can hear), and the recurring failure it kept naming — *a number that looks like the quantity and is not* — was found seven times, twice in the instruments themselves. |
