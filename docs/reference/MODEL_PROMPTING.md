...
> `local_catalog.RETIRED_IDS`, so `deploy/local-models-sync.sh` force-uninstalls and prunes
> their weights from any box still holding them on the next update (no-shell, CLAUDE.md #10).

> **Applied (2026-08-14):** added `Qwen/Qwen3.8-27B` to the local catalog (Q8 + Q4 twin,
> `qwen3.8-27b` / `qwen3.8-27b-q4`) — the newer-generation dense-27B vision hybrid.
> Qwen hybrid sampling split (non-thinking 0.7/0.80/pp 1.5, thinking 1.0/0.95), carried on
> the catalog entry; see the per-model table below.
