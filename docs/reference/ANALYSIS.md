...
repairs affected supersession chains **[decided]**. Delete = gone; the note row
...
- **The corpus rebuild sweep is the one exception to that, and it is an
  ...
  destructive half with four exemptions that answer two of the rejection's
  ...
  untouched, since nothing re-derives them — as are **note conversations**, for
  the same reason and one stronger: a thread holds the owner's answers to the
  agent's clarification questions, which no re-derive from the notes can
  reconstruct, and destroying one would orphan a question still waiting in the
  notes inbox. Both halves of "a review item that
