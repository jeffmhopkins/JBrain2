...
affected supersession chains **[decided]**. The note's own **clarification
blocks** go with them — not derived, but part of the note's text. Delete =
gone; the note row itself stays soft-deleted, which is why both the block
purge and the episode purge are explicit statements rather than FK cascades.
...
- **The corpus rebuild sweep is the one exception to that, and it is an
  ...
  dangling pointer whose reopen silently no-ops; agent episodes are
  untouched, since nothing re-derives them; and the notes' **clarification
  blocks** are untouched, since they are source the owner typed and the whole
  premise of the sweep is re-deriving *from* the notes — a rebuild that took
  them would break re-derivability corpus-wide and silently. Both halves of "a review item that
