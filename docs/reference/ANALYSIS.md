...
- **Still open: the review inbox's merge accept does not re-check its pair.**
  `resolve_review`'s `merge_proposal` arm folds `entity_b` into the payload's `entity_a`
  with no status check and no `plan_merge` re-rank, unlike the agent's `merge_entities`
  (which no-ops when either side is already merged). Two overlapping open cards resolved
  in sequence therefore repoint the second pair's rows onto a tombstone. Same class of
  hole, different write.
