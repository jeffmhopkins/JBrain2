...
- **`note_ingest`** — the **note conversation**
  ...
  **are** listed: the PWA's Full Brain tab carries `note_ingest` beside the curator
  ...
  `current_time` inherited unchanged. Once the owner replies it holds **thirteen** —
  those six plus `assert_fact`, `correct_fact`, `merge_entities`, `prefs_write`,
  `search`, `read_note` and `relate`. `assert_fact` is on the REPLY side since R3 of
  `AGENT_INGEST_REWRITE.md`, and the reason is the settle rather than the surface: that
  pass's sweep is derived from its closing reading, so a second fact verb beside the
  reading could write a fact the sweep then retracted. Its reply-turn job — record one
  more thing the owner just told me — is incremental, and a reply turn that only added a
  fact closes no reading and sweeps nothing. Nothing outward-facing is in either: the owner replying does
