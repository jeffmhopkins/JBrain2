...
10. **Untrusted-origin content triggers exactly one background job, on a surface with
    ...
    `close_reading`, `assert_fact`, `find_entity`, `read_entity`, `current_time` — on the
    unattended pass
...
- **`note_ingest`** — the **note conversation**
  ...
  **are** listed: the PWA's Full Brain tab carries `note_ingest` beside the curator
  ...
  below). Reading a note on its own it holds **seven**: the note-bound graph
  writes `resolve_entity`, `close_reading` and `assert_fact`, `ask_owner`, and
  `find_entity` / `read_entity` / `current_time` inherited unchanged. Once the owner
  replies it holds **thirteen** — those seven plus `correct_fact`, `merge_entities`,
  `prefs_write`, `search`, `read_note` and `relate`. Nothing outward-facing is in either: the owner replying does
  ...
  allowlist is the **six** of `NOTE_INGEST_THIRD_PARTY_TOOLS`, and it applies on BOTH
  ...
  unattended seven minus `ask_owner`, and it does not widen on reply, because D8's premise
