...
- **`note_ingest`** — the **note conversation**
  ...
  **Thirteen is the CEILING, not what every reply turn holds**, and the subtraction is
  `agents.narrow_for_unprompted_reply`. `assert_fact` is bound only on a turn whose words
  ...
  dropped), never the thread's state — a `waiting_on_owner` turn fails it three ways, the
  commonest being §3b I7's designed send, where the tapped answers land and the free text
  typed beside them has no open question to pair with. It is applied in `api/agent.py` on
