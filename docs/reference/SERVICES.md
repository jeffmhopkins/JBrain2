...
| `searxng` | SearXNG | Self-hosted metasearch backing `jerv`'s `web_search` and `news_search` (the news category, dated leads). Only the KB-blind `jerv` reaches it. | internal |
