...
| `GET /host` | Live host memory/swap/disk/load + **per-container RSS** (biggest first), proxied from the supervisor. Attributes the unified-memory total to specific services — the `local-llm` container's number includes the loaded model RSS (the gateway runs `--no-mmap`), so it answers "what is using the box's RAM" the read-only meter can only total. |
