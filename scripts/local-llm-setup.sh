...
# llama-swap's ${PORT} macro isn't substituted by every build (older binaries
# pass it through literally, so llama-server gets a non-numeric --port and dies
# with "stoi"). Assign concrete, distinct upstream ports instead and point each
# model's proxy at it — distinct because the resident group runs models
# concurrently, so they can't share a port. llama-swap and llama-server share the
# container, so 127.0.0.1 is both correct and tighter than binding 0.0.0.0.
