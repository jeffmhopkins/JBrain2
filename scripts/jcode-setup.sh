...
say ""
say "Web preview is ON with code mode — no extra env var. A session's dev server is served"
say "at <slug>-preview.<host> through the box's own tunnel + the api↔jcode proxy, once a"
say "base host is configured with 'jbrain enable-jcode-preview'. Nothing is exposed until"
say "you open a preview on a session. To withhold the capability: set JCODE_PREVIEW_ENABLED=false"
say "in .env and re-run this script (or 'jbrain up jcode')."
