...
say "  - grok   (@vibe-kit/grok-cli) → straight at the gateway's OpenAI /v1 (no shim)"
say ""
...
say ""
say "Web preview (Wave J4) is ON with code mode — no extra env var. The owner opens an"
say "ephemeral Cloudflare tunnel per session from the Preview tab; it's reachable by"
say "anyone with the (random) URL and dies with the session. Nothing is exposed until you"
say "open one. To withhold the capability: set JCODE_PREVIEW_ENABLED=false in .env and"
say "re-run this script (or 'jbrain up jcode')."
