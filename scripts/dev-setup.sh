...
# session shell (`@anthropic-ai/claude-code` and `grok-dev`, the latter on a bundled Bun
# runtime) and the on-box coder model — is provided by the jcode IMAGE (jcode/Dockerfile)
# and enabled SEPARATELY by
