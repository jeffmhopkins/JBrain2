...
# session shell (`@anthropic-ai/claude-code` and `@vibe-kit/grok-cli`) and the on-box
# coder model — is provided by the jcode IMAGE (jcode/Dockerfile) and enabled SEPARATELY by
