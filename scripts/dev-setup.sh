...
BYPARR_IMAGE="${BYPARR_IMAGE:-ghcr.io/thephaseless/byparr:latest}"  # opt-in bot-challenge solver (`solver` profile)
...
if docker info >/dev/null 2>&1; then
  ...
  # Pre-pull the opt-in bot-challenge solver image (Byparr, `solver` profile) so enabling
  # the web_fetch solver fallback isn't a cold pull; best-effort. CI never runs the profile
  # (web fetch is faked via MockTransport), so this is local/dev convenience only.
  if ! docker image inspect "$BYPARR_IMAGE" >/dev/null 2>&1; then
    log "pre-pulling $BYPARR_IMAGE (opt-in solver profile)"
    for _ in 1 2 3; do docker pull "$BYPARR_IMAGE" >/dev/null 2>&1 && break; sleep 10; done \
      || log "WARNING: could not pre-pull $BYPARR_IMAGE — it will pull when the profile is enabled"
  fi
else
...
# --- Code mode / jcode (Python deps synced above; CLI + model are opt-in) ---
...
# no network. The box-only runtime — the coding CLI the owner runs in the session
# shell (xAI's official Grok Build binary) and the on-box coder model — is provided by
# the jcode IMAGE (jcode/Dockerfile) and enabled SEPARATELY by
