...
describe("heatOvalPoints", () => {
  it("leaves a near-stationary fix as a single round blob", () => {
    const pts = heatOvalPoints([moving(0.5, 90)], [at(40, -74)], 0.4);
    expect(pts).toEqual([[40, -74, 0.4]]);
  });

  it("smears a moving fix into satellites along its heading (oval)", () => {
    // Heading due east (90°): satellites shift in longitude, not latitude.
    const pts = heatOvalPoints([moving(10, 90)], [at(40, -74)], 0.4);
    expect(pts).toHaveLength(5); // the fix + 4 satellites
    ...
    for (const [lat, lon, w] of sats) {
      ...
      expect(w).toBe(0.2); // satellites carry half weight
    }
    // Symmetric fore/aft about the fix.
    const lons = sats.map((p) => p[1]).sort((a, b) => a - b);
    expect((lons[0] as number) + (lons[3] as number)).toBeCloseTo(2 * -74, 6);
  });

  it("caps the stretch so highway speed doesn't streak across the map", () => {
    const fast = heatOvalPoints([moving(200, 0)], [at(40, -74)], 0.4); // 200 m/s, due north
    const reachLat = Math.max(...fast.slice(1).map((p) => Math.abs(p[0] - 40)));
    // Capped at 120 m ≈ 0.00108°, far below 200*4 s of un-capped travel.
    expect(reachLat).toBeCloseTo(120 / 111_320, 6);
  });
});
