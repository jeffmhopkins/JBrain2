...
export function LocationMapTab({ deps }: { deps: LocationDeps | undefined }) {
  ...
  return (
    <div className="loc-map">
      ...
      <div className="loc-places">
        <div className="loc-places-head">
          <span>Places</span>
          <button type="button" className="loc-places-add" onClick={() => setEditing("new")}>
            ＋ Add place
          </button>
        </div>
            ...
            {meta.places.map((p) => (
              ...
              >
                ...
                <span className="loc-place-meta">
                  {p.center && p.radius_m !== null ? `${Math.round(p.radius_m)} m radius` : "area"}
                </span>
              </button>
            ))}
          </div>
