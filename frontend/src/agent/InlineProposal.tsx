...
interface Props {
   ...
   * follow-up turn (fb.send(text, { proposalOutcome: true })). */
...
export function InlineProposal({
  ...
  const [resolved, setResolved] = useState<EnactResult | null>(null);
  ...
  const leaves = detail?.nodes.filter((n) => n.type === "leaf") ?? [];
  ...
  function toggle(id: string, next: LeafState): void {
    ...
    setOpenReason(next === "out" ? id : null);
  }
  ...
  function commitEdit(id: string, value: string): void {
    ...
    if (value.trim() && value.trim() !== original.trim()) {
      setEdits((e) => ({ ...e, [id]: value.trim() }));
      setState((s) => ({ ...s, [id]: "in" })); // an edit implies keep
    }
  }
  ...
  async function enact(): Promise<void> {
    ...
    setBusy(true);
    try {
      // Push every decision, then the edits, then enact — the proposal stays 'staged'
      // until enact, so an edit lands before the executor reads preview.body.
  ...
  if (detail === null) {
    return <div className="fb-inline-prop loading">Loading proposal…</div>;
  }
  ...
  if (resolved !== null) {
    return (
      <output className="fb-inline-prop done">
        ...
        <span>
          Enacted {resolved.enacted.length} operation{resolved.enacted.length === 1 ? "" : "s"}
          {resolved.held.length > 0 && ` · ${resolved.held.length} held`}
          <span className="ip-sub">one message sent to the assistant</span>
        </span>
      </output>
    );
  }
        ...
        {topLevel.map((n) =>
          n.type === "group" ? (
            ...
            <Leaf
              ...
              reasonOpen={openReason === n.id}
              held={held(n.id)}
              busy={busy}
              onToggle={toggle}
              onStartEdit={setEditing}
              onCommitEdit={commitEdit}
              onReason={(v) => setReasons((r) => ({ ...r, [n.id]: v }))}
            />
          ),
        )}
      </div>
      <div className="ip-foot">
        <span className="ip-tally">
      ...
      <div className="ip-group-body">
        {children.map((n) => (
          <Leaf
            key={n.id}
            node={n}
            state={rest.state[n.id] ?? "in"}
            edited={rest.edits[n.id]}
            reason={rest.reasons[n.id] ?? ""}
            editing={rest.editing === n.id}
            reasonOpen={rest.openReason === n.id}
            held={rest.held(n.id)}
            busy={rest.busy}
            onToggle={rest.onToggle}
            onStartEdit={rest.onStartEdit}
            onCommitEdit={rest.onCommitEdit}
            onReason={(v) => rest.onReason(n.id, v)}
          />
        ))}
      </div>
    </div>
