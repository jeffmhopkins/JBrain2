...
import type { Domain, FactWrite } from "./types";
...
export const WRITE_TOOLS: ReadonlySet<string> = new Set(["assert_fact", "resolve_entity"]);
...
export function tallyWrites(facts: readonly FactWrite[]): WriteTally {
  ...
  for (const f of facts) {
    if (f.status === "replaced") tally.replaced += 1;
    else if (f.status === "held") tally.held += 1;
...
export function writeVerb(fact: FactWrite): string {
  if (fact.status === "replaced") return "replaced";
  if (fact.status === "held") return "held";
  return "written";
}
