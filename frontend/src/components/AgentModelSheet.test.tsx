...
// box the tests exercise (one loaded reasoning model, one not loaded).
vi.mock("../api/client", () => ({
  api: {
...
describe("AgentModelSheet", () => {
  ...
  it("carries an armed reasoning level onto the model pick", async () => {
    ...
    // A reasoning model is offerable → the level control shows. Arming a level does
  ...
  it("re-applies a level change live to the active reasoning pick", async () => {
    ...
    // Tapping a level applies to the active pick without closing; "Auto" clears back
  ...
  it("does not carry a level onto a non-reasoning model", async () => {
    ...
    render(
      <AgentModelSheet
        // The vision model is loaded this time, so a non-reasoning row is pickable.
