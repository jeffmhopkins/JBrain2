// Move-to-folder bottom sheet for the Research Library's Reports tab — the report sibling
// of MoveTaskSheet. Filing a report into a folder is a deliberate menu pick from the row's
// ⋯ action sheet, never a drag: the owner's folders as rows (the current one ticked), an
// "Ungrouped" escape, and a "New folder…" row that files into a folder it creates in the
// same tap. Composes the shared Sheet.

import { useState } from "react";
import type { ReportGroup } from "../api/client";
import { Sheet } from "./Sheet";
import { CheckIcon, PlusIcon } from "./icons";

interface MoveReportSheetProps {
  reportTitle: string;
  currentGroupId: string | null;
  groups: ReportGroup[];
  /** Move into an existing folder (null = Ungrouped). */
  onMove: (groupId: string | null) => void;
  /** Create a folder by this name and move the report into it. */
  onCreateAndMove: (name: string) => void;
  onClose: () => void;
}

export function MoveReportSheet({
  reportTitle,
  currentGroupId,
  groups,
  onMove,
  ...
  return (
    <Sheet title="Move to folder" onClose={onClose}>
      <p className="movetask-lead">
        Move <b>{reportTitle}</b> to…
      </p>
      <div className="movetask-rows" aria-label="Destination folder">
        {groups.map((g) => (
          <button
            key={g.id}
            type="button"
        ...
        {creating ? (
          ...
          >
            <PlusIcon size={16} />
            <span className="movetask-name">New folder…</span>
          </button>
        )}
      </div>
