...
show_part = false;
$fn = 64; $k = 0.22;
v = "right"; x = 2.6; t = 0.45;
m_inv = [[sc, 0, ss, -ss * stand_zh], [0, 1, 0, 0], [-ss, 0, sc, -sc * stand_zh], [0, 0, 0, 1]];
module slice() intersection() { children(); translate([1.6, 15, -6]) cube([0.4, 10, 13]); }
...
color("LightGray") translate([1.7, plate_y / 2 - 1.3, body_h]) cube([0.3, 1.3, 3.5]);
label([x, 23.9, 5.8], "front shell (ghost)", v, t, "left", c = "DimGray");
// The lock screw.
// The lock screw lies just beside this cut, at x = 0.
color("Goldenrod", 0.45) translate([0, plate_y / 2 + 1.6, lock_z]) rotate([90, 0, 0]) cylinder(d = 2, h = 7.6);
color("Goldenrod", 0.45) translate([0, plate_y / 2 + 1.6, lock_z]) rotate([-90, 0, 0]) cylinder(d = 3.8, h = 1.2);
