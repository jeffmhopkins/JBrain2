...
def render(
    ...
    *,
    ...
    non-swapping group so staging keeps them co-resident."""
    windows = windows or {}
    ...
    # A non-swapping group keeps a chosen set of models co-resident (hot paths skip
    # the cold-load on task switch); everything else stays swappable, one at a time.
    # Two sources union here: the install-time `recommended` set (only when
    # resident_group is on) and the operator's runtime staged set (`pinned`, catalog
    # ids) — so staging a model pins it resident, and unstaging frees it to swap.
    # Emitted for one or more members: a lone staged model still pins itself.
