        ...
        *,
        ...
        async with scoped_session(self._maker, ctx) as session:
            ...
            run = Run(
                ...
                # persist() runs only on a COMMITTED plan (the caller short-circuits
                # on plan.rejected — nothing was written, so there is nothing to
                # record). 'rejected' isn't a valid runs.status anyway (the CHECK
                # allows running/done/error), so a committed run is always 'done'.
                status="done",
