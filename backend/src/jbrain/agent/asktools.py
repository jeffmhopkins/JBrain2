...
from jbrain.agent.toolregistry import RegisteredTool, ToolHandler
...
which is exactly what building this registry by name avoids."""


def note_conversation_tools(
    maker: async_sessionmaker[AsyncSession], tools_dir: Path | None = None
) -> list[RegisteredTool]:
    """The note conversation's registry contents, loaded by name.

    Not `load_registry(tools_dir, ...)`: that globs the WHOLE sidecar directory and
    demands a handler for every one of the 116 files in it. The worker's registry is the
    second lock after D16's allowlist (`converse.note_converse_handler`), so it is built
    from an explicit list — a tool reaches this persona only by being named here AND
    admitted by `agents.NOTE_INGEST_TOOLS`."""
    directory = tools_dir or TOOLS_DIR
    handlers = build_ask_owner_handlers(maker)
    return [
        RegisteredTool(toolfile=load_tool(directory / f"{name}.tool"), handler=handler)
