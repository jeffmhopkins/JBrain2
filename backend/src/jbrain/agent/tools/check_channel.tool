...
params:
  ...
  properties:
    ...
    full_descriptions:
      type: boolean
      description: >-
        Optional — return each upload's COMPLETE uploader description instead of the default
        one-line teaser. Off by default so a listing of many uploads stays compact; turn it on
        when you need the whole blurb to judge borderline videos.
    limit:
      type: integer
...
like the last 7 days), and set full_descriptions when the one-line teaser isn't enough to judge.
Pass a channel id (a UC… id) or an @handle, never a full URL. Results already in the library are
omitted, so analysing a returned video never repeats work.
