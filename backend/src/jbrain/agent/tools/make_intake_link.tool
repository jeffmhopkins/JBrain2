...
params:
  ...
  properties:
    subject_id:
      type: string
      description: >-
        Optional. The id of an EXISTING subject this link collects information ABOUT (the
        person the captured facts are attributed to). Resolve it from the owner's own
        subjects before setting it — never invent one. OMIT it entirely for a general
        collection that isn't about a specific person (a recipe, general info, a survey).
    domain:
      type: string
  ...
  required: [domain, fields_brief, max_runs, bind_on_first]
---
...
else (a relative's medical history, a contractor's quote, a new contact's details) — or a
general collection about no one in particular (a favorite recipe, general info). Fill the
brief by interviewing the owner: which subject the information is about (omit it if it's
not about a specific existing person), which domain, exactly what to collect, and whether
it's for one person or several. Draft a warm opening
