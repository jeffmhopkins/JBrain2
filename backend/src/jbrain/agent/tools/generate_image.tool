...
params:
  ...
  properties:
    ...
    speed:
      type: string
      enum: [dreamshaper, fast, quality]
      description: Speed vs. fidelity. quality (the default) runs the full local model at 20–40 diffusion steps — best detail, but a slow render (a minute or more). fast runs the same model through a 4-step Lightning distillation — much quicker at slightly lower detail, but still high quality. dreamshaper is a tiny separate SDXL model that renders in seconds — lowest fidelity, but near-instant; reach for it when the owner just wants to see something immediately. Prefer fast for casual or exploratory requests and quality for a finished, detailed piece.
    negative_prompt:
      type: string
    ...
    steps:
      type: integer
      description: Number of diffusion steps on the quality path, 20–40 (default 20). 20 is a quick but finished render; raise toward 40 for more detail at more time. Values outside the range are clamped. Ignored when speed is fast or dreamshaper (those run at a fixed step count).
    seed:
      type: integer
...
takes a moment, a fast render (speed: fast) is much quicker, and speed: dreamshaper
returns in seconds — reach for the quicker tiers when the owner wants something
fast or you are just exploring an idea. The
