...
params:
  ...
  properties:
    id:
      type: string
      description: The id of a saved report (from a research_report action=list or action=search result).
    question:
      type: string
...
me my research on X", "pull up that report"), after research_report(action=list) or
action=search has found which one they mean. Pass its id (or the exact question).

Prefer research_report(action=read) when the owner wants the CONTENT in words (to summarize or
