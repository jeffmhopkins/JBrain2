...
params:
  ...
  properties:
    url:
      type: string
      description: The URL (or id) of a library video to remove — e.g. one an external_video action=search result linked to.
  required: [url]
---
...
external_video(action=search) first, then pass its URL.
