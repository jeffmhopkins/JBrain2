...
params:
  ...
  properties:
    id:
      type: string
      description: The id of a saved report to remove (from a research_report action=list or action=search result).
    question:
      type: string
...
a report from their library; find the right one with research_report(action=list) or
action=search first, then pass its id (or the exact question).
