...
def serials_in(usb_payload: object) -> list[str]:
    ...
    and two copies were wrong. `api/debug.py` renders the same payload into a full
    verdict; `test_tuner_range.py` feeds one fixture to both and fails if they ever
    ...
    exists to remove. A device with no serial is skipped: it cannot be addressed with
    `-d serial=`, so naming it would promise a selection that cannot be made."""
    if not isinstance(usb_payload, dict):
        return []
