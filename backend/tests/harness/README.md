...
| 5 | **No `confidence`** | The tool surface has no confidence field, so the model's own 0.25 self-report is inexpressible and this scenario cannot pass. The guard ITSELF is live: `assert_fact` stamps `self_confidence=confidence`, so an unattested write (0.4, below `LOW_CONFIDENCE`) is held — proved by `test_note_graph_write_pg.py`. What is missing is a channel for a self-report LOWER than the quote check implies. The one **safety** gap of the six. |
