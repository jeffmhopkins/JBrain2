...
def _router(classifications: list[dict]) -> tuple[LlmRouter, FakeLlmClient]:
    fake = FakeLlmClient(responses=[json.dumps({"classifications": classifications})])
...
async def test_files_newest_day_and_leaves_older_mail_in_inbox() -> None:
    ...
    # Batch is sorted by id, so index 0..2 == m1, m2, m3. m3 is spam below the floor.
    router, llm = _router(
        [
            {"index": 0, "bucket": "high", "confidence": 0.9},
            {"index": 1, "bucket": "low", "confidence": 0.95},
            {"index": 2, "bucket": "spam", "confidence": 0.3},
        ]
    )

    await InboxTriage(_factory(fake), router).run({})

    # One LLM call (single chunk); the body was sent, not just the snippet.
    assert len(llm.calls) == 1
    assert "invoice" not in llm.calls[0]["system"]  # subjects ride in user_text, not system
...
async def test_unclassified_message_stays_in_inbox() -> None:
    ...
    # The model returns a verdict for index 0 only; m2 must be left alone.
    router, _ = _router([{"index": 0, "bucket": "high", "confidence": 0.8}])
    await InboxTriage(_factory(fake), router).run({})
    assert _names_on(fake, "m1") == {"triaged/high"}
...
async def test_archived_mail_drops_out_of_inbox_on_rerun() -> None:
    ...
    router1, _ = _router([{"index": 0, "bucket": "high", "confidence": 0.9}])
    ...
    router2, llm2 = _router([{"index": 0, "bucket": "low", "confidence": 0.9}])
