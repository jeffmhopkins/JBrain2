...
def test_chat_streams_text_then_done(
    ...
    assert runlog.started == [("sess-1", "agent-system-v14")]
...
def test_chat_records_a_tool_calls_text_offset(
    ...
    client.app.state.agent_registry = registry_with_tool("canvas", gen)  # type: ignore[attr-defined]
    client.app.state.llm_router = stream_router(  # type: ignore[attr-defined]
        [
            LlmTurn("", (ToolCall("c1", "canvas", {"prompt": "x"}),), "tool_use", LlmUsage(1, 1)),
...
def test_chat_runs_the_selected_agents_prompt_and_only_its_tools(
    ...
    assert ("sess-j", "agent-jerv-v53") in client.app.state.agent_runlog.started  # type: ignore[attr-defined]
