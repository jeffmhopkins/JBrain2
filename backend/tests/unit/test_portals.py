...
from jbrain.web.portals import FlSunbizResolver, advertised_capabilities, select_resolvers
...
def test_registry_dispatch_by_jurisdiction_kind_and_key() -> None:
    ...
    assert select_resolvers(resolvers, key="fl_business") == (sunbiz,)
    assert select_resolvers(resolvers, jurisdiction="FL", kind="license") == ()
    assert select_resolvers(resolvers, jurisdiction="TX", kind="business") == ()
    assert advertised_capabilities(resolvers) == "FL/business"
