...
# --- the owner's callsign ------------------------------------------------------------
# It lives on the Radio screen rather than in Settings because it is meaningless
# anywhere else on this box: its whole job is to divide a heard log into his traffic and
# everybody else's. A FILTER, never an identity — a callsign forges trivially.
...
class _Store:
    ...
    async def owner_callsign(self, _ctx: Any) -> str | None:
        return self.value

    async def upsert(self, _ctx: Any, key: str, value: str) -> None:
        self.wrote.append((key, value))
        self.value = value or None


def _req(store: _Store) -> Any:
    from types import SimpleNamespace

    return SimpleNamespace(app=SimpleNamespace(state=SimpleNamespace(settings_store=store)))


async def test_an_unset_callsign_reads_as_none(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(sdr_api, "ctx_for", lambda _o: object())

    out = await sdr_api.read_callsign(_req(_Store()), OWNER)  # type: ignore[arg-type]

    # Unset is a real state with a real consequence — "my traffic" cannot be computed —
    # so it is reported, never guessed at.
    assert out.callsign is None


async def test_a_callsign_is_stored_upper_cased(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(sdr_api, "ctx_for", lambda _o: object())
    store = _Store()

    out = await sdr_api.write_callsign(
        _req(store), OWNER, sdr_api.CallsignBody(callsign=" ke8xyz-9 ")
    )

    # Upper case is how it travels on the air; storing it as typed would make every
    # comparison a case-folding question later.
    assert out.callsign == "KE8XYZ-9"
    assert store.wrote == [("owner_callsign", "KE8XYZ-9")]


async def test_an_empty_callsign_clears_it(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(sdr_api, "ctx_for", lambda _o: object())
    store = _Store("KE8XYZ")

    out = await sdr_api.write_callsign(_req(store), OWNER, sdr_api.CallsignBody(callsign=""))

    assert out.callsign is None


@pytest.mark.parametrize("bad", ["KE8 XYZ", "KE8XYZ!", "KE8XYZé"])
async def test_a_mangled_callsign_is_refused_not_silently_cleaned(
    monkeypatch: pytest.MonkeyPatch, bad: str
) -> None:
    monkeypatch.setattr(sdr_api, "ctx_for", lambda _o: object())
    store = _Store()

    # Refused rather than stripped: a callsign that quietly lost a character would
    # filter for a station that does not exist, and the owner would see an empty log
    # and conclude the radio was deaf.
    with pytest.raises(HTTPException) as raised:
        await sdr_api.write_callsign(_req(store), OWNER, sdr_api.CallsignBody(callsign=bad))
